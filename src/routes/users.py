...
from src.chemas import UserResponse
...
async def read_users_me(current_user: User = Depends(auth_service.get_current_user)):
    """
    The read_users_me function is a GET endpoint that returns the user`s information.
    It uses the auth_service to get the current user, and then returns it.

    :param current_user: User: Get the current user
    :return: The currently logger in user
    :doc-author: Trelent
    """
    return current_user
